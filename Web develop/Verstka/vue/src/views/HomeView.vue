<template>

<body>
    <nav class="menu">
      <div class="menu-left">
        <svg class="menu-left-logo" width="44" height="38" viewbox="0 0 44 38">
          <rect width="44" height="38" fill="url(#pattern0_128_53)" />
          <defs>
            <pattern id="pattern0_128_53" patternContentUnits="objectBoundingBox" width="1" height="1">
              <use xlink:href="#image0_128_53" transform="scale(0.0227273 0.0263158)" />
            </pattern>
            <image id="image0_128_53" width="44" height="38"
              xlink:href="data:image/png;base64," />
          </defs>
        </svg>
        <svg class="menu-left-search" viewbox="0 0 26 27">
          <path
            d="M18.0596 17.6259C19.6713 15.8658 20.6282 13.6048 20.7698 11.2225C20.9113 8.84018 20.2288 6.48173 18.8369 4.54318C17.445 2.60463 15.4285 1.20404 13.126 0.576619C10.8234 -0.0508009 8.3751 0.13316 6.19217 1.09761C4.00923 2.06205 2.22463 3.74825 1.13804 5.87302C0.0514473 7.9978 -0.270946 10.4318 0.224996 12.7661C0.720938 15.1005 2.00501 17.1932 3.86158 18.6927C5.71814 20.1922 8.03413 21.0072 10.4206 21.0009C12.673 21.004 14.8645 20.27 16.6606 18.9109L24.4086 26.7179C24.4941 26.807 24.5965 26.8781 24.7099 26.927C24.8232 26.9759 24.9452 27.0017 25.0686 27.0029C25.1923 27.0033 25.3148 26.9782 25.4283 26.9292C25.5419 26.8801 25.6441 26.8082 25.7286 26.7179C25.9025 26.537 25.9997 26.2958 25.9997 26.0449C25.9997 25.794 25.9025 25.5528 25.7286 25.3719L18.0596 17.6259ZM1.88662 10.5009C1.89946 8.81563 2.41094 7.17187 3.35659 5.77685C4.30224 4.38183 5.63972 3.29801 7.20044 2.662C8.76115 2.02599 10.4752 1.86627 12.1266 2.20298C13.7779 2.53969 15.2926 3.35775 16.4797 4.55404C17.6668 5.75033 18.4732 7.27129 18.7972 8.92519C19.1212 10.5791 18.9483 12.2919 18.3002 13.8476C17.6522 15.4034 16.5581 16.7325 15.1559 17.6674C13.7536 18.6023 12.1059 19.1011 10.4206 19.1009C8.14916 19.0901 5.97482 18.1784 4.37484 16.566C2.77486 14.9537 1.87998 12.7724 1.88662 10.5009Z"
            fill="#E8E8E8" />
        </svg>
      </div>
      <div class="menu-right">
        <svg class="menu-right-burger" width="32" height="23" viewbox="0 0 32 23">
          <path d="M0 23V20.31H32V23H0ZM0 12.76V10.07H32V12.76H0ZM0 2.69V0H32V2.69H0Z" fill="#E8E8E8" />
        </svg>
        <svg class="menu-right-account" width="29" height="29" viewbox="0 0 29 29">
          <path
            d="M14.5 19.937C19 19.937 22.656 15.464 22.656 9.968C22.656 4.472 19 0 14.5 0C13.3631 0.0217413 12.2463 0.303398 11.2351 0.823397C10.2239 1.34339 9.34507 2.08794 8.66602 3C7.12663 4.99573 6.30819 7.45381 6.34399 9.974C6.34399 15.465 10 19.937 14.5 19.937ZM14.5 1.813C18 1.813 20.844 5.472 20.844 9.969C20.844 14.466 17.998 18.125 14.5 18.125C11.002 18.125 8.15603 14.465 8.15503 9.969C8.15403 5.473 11 1.813 14.5 1.813ZM20.844 18.125C20.6036 18.125 20.373 18.2205 20.203 18.3905C20.033 18.5605 19.9375 18.7911 19.9375 19.0315C19.9375 19.2719 20.033 19.5025 20.203 19.6725C20.373 19.8425 20.6036 19.938 20.844 19.938C22.526 19.9399 24.1386 20.6088 25.3279 21.7982C26.5172 22.9875 27.1861 24.6 27.188 26.282C27.1875 26.5221 27.0918 26.7523 26.922 26.9221C26.7522 27.0918 26.5221 27.1875 26.282 27.188H2.71997C2.47985 27.1875 2.24975 27.0918 2.07996 26.9221C1.91016 26.7523 1.81449 26.5221 1.81396 26.282C1.81608 24.6001 2.48517 22.9877 3.67444 21.7985C4.86371 20.6092 6.47608 19.9401 8.15796 19.938C8.39824 19.938 8.62868 19.8425 8.79858 19.6726C8.96849 19.5027 9.06396 19.2723 9.06396 19.032C9.06396 18.7917 8.96849 18.5613 8.79858 18.3914C8.62868 18.2215 8.39824 18.126 8.15796 18.126C5.99541 18.1279 3.92201 18.9875 2.39258 20.5164C0.863144 22.0453 0.00264777 24.1185 0 26.281C0.000794067 27.0019 0.287502 27.693 0.797241 28.2027C1.30698 28.7125 1.99811 28.9992 2.71899 29H26.282C27.0027 28.9989 27.6936 28.7121 28.2031 28.2024C28.7126 27.6927 28.9992 27.0017 29 26.281C28.9974 24.1187 28.1372 22.0457 26.6083 20.5168C25.0793 18.9878 23.0063 18.1276 20.844 18.125Z"
            fill="#E8E8E8" />
        </svg>
        <svg class="menu-right-cart" width="32" height="29" viewbox="0 0 32 29">
          <path
            d="M26.2009 29C25.5532 28.9738 24.9415 28.6948 24.4972 28.2227C24.0529 27.7506 23.8114 27.1232 23.8245 26.475C23.8376 25.8269 24.1043 25.2097 24.5673 24.7559C25.0303 24.3022 25.6527 24.048 26.301 24.048C26.9493 24.048 27.5717 24.3022 28.0347 24.7559C28.4977 25.2097 28.7644 25.8269 28.7775 26.475C28.7906 27.1232 28.549 27.7506 28.1047 28.2227C27.6604 28.6948 27.0488 28.9738 26.401 29H26.2009ZM6.75293 26.32C6.75293 25.79 6.91011 25.2718 7.20459 24.8311C7.49907 24.3904 7.91764 24.0469 8.40735 23.844C8.89705 23.6412 9.43594 23.5881 9.95581 23.6915C10.4757 23.7949 10.9532 24.0502 11.328 24.425C11.7028 24.7998 11.9581 25.2773 12.0615 25.7972C12.1649 26.317 12.1118 26.8559 11.9089 27.3456C11.7061 27.8353 11.3626 28.2539 10.9219 28.5483C10.4812 28.8428 9.96304 29 9.43298 29C9.08087 29.0003 8.73212 28.9311 8.40674 28.7966C8.08136 28.662 7.78569 28.4646 7.53662 28.2158C7.28755 27.9669 7.09001 27.6713 6.9552 27.3461C6.82039 27.0208 6.75098 26.6721 6.75098 26.32H6.75293ZM10.553 20.686C10.2935 20.6868 10.0409 20.6024 9.83411 20.4457C9.62727 20.2891 9.47758 20.0689 9.40796 19.819L4.57495 2.36401H1.18201C0.868521 2.36401 0.567859 2.23947 0.346191 2.01781C0.124523 1.79614 0 1.49549 0 1.18201C0 0.868519 0.124523 0.567873 0.346191 0.346205C0.567859 0.124537 0.868521 5.81268e-06 1.18201 5.81268e-06H5.46301C5.7225 -0.00080736 5.97504 0.0837201 6.18176 0.240568C6.38848 0.397416 6.53784 0.617884 6.60693 0.868006L11.4399 18.323H24.6179L29.001 8.27501H14.401C14.2428 8.27961 14.0854 8.25242 13.9379 8.19507C13.7904 8.13771 13.6559 8.05134 13.5424 7.94108C13.4288 7.83082 13.3386 7.69891 13.277 7.55315C13.2154 7.40739 13.1836 7.25075 13.1836 7.0925C13.1836 6.93426 13.2154 6.77762 13.277 6.63186C13.3386 6.4861 13.4288 6.35419 13.5424 6.24393C13.6559 6.13367 13.7904 6.0473 13.9379 5.98994C14.0854 5.93259 14.2428 5.90541 14.401 5.91001H30.814C31.0097 5.90996 31.2022 5.95866 31.3744 6.05172C31.5465 6.14478 31.6928 6.27926 31.7999 6.44301C31.9078 6.60729 31.9734 6.79569 31.9908 6.99145C32.0083 7.18721 31.9771 7.38424 31.9 7.565L26.495 19.977C26.4026 20.1876 26.251 20.3668 26.0585 20.4927C25.866 20.6186 25.641 20.6858 25.411 20.686H10.553Z"
            fill="#E8E8E8" />
        </svg>
      </div>
    </nav>
    <main class="main">
      <article class="main-first">
        <h1 class="main-first-title">
          <div class="main-first-title-divider"></div>
          <span class="main-first-title-words">the brand</span>
          <span class="main-first-title-start">of luxerious</span>
          <span class="main-first-title-end">fashion</span>
        </h1>
      </article>
      <figure class="main-discount">
        <aside class="main-discount-up">
          <button class="main-discount-up-button">
            <span class="main-discount-up-button-percent">30% off</span>
            <span class="main-discount-up-button-human">for woman</span>
          </button>
          <button class="main-discount-up-button">
            <span class="main-discount-up-button-percent">hot deal</span>
            <span class="main-discount-up-button-human">for men</span>
          </button>
          <button class="main-discount-up-button">
            <span class="main-discount-up-button-percent">new arrivals</span>
            <span class="main-discount-up-button-human">for kids</span>
          </button>
        </aside>
        <button class="main-discount-down">
          <span class="main-discount-down-percent">luxirous &#38; trendy </span>
          <span class="main-discount-down-text">for kids</span>
        </button>
      </figure>
      <section class="main-featured">
        <div class="main-featured-title">featured</div>
        <div class="main-featured-subtitle">Shop for items based on what we featured in this week</div>
        <figure class="main-featured-cards">
          <button class="main-featured-cards-btn">
            <img class="main-featured-cards-btn-img" src="/img/clothes1.png" width="360" height="420">
            <span class="main-featured-cards-btn-title">ellery x m`o capsule</span>
            <span class="main-featured-cards-btn-text">Known for her sculptural takes on
              traditional tailoring. Australian arbiter of cool Kym Ellery teams up with Moda
              Operandi.</span>
            <span class="main-featured-cards-btn-price">$52.00</span>
          </button>
          <button class="main-featured-cards-btn">
            <img class="main-featured-cards-btn-img" src="/img/clothes2.png" width="360" height="420">
            <span class="main-featured-cards-btn-title">ellery x m`o capsule</span>
            <span class="main-featured-cards-btn-text">Known for her sculptural takes on
              traditional tailoring. Australian arbiter of cool Kym Ellery teams up with Moda
              Operandi.</span>
            <span class="main-featured-cards-btn-price">$52.00</span>
          </button>
          <button class="main-featured-cards-btn">
            <img class="main-featured-cards-btn-img" src="/img/clothes3.png" width="360" height="420">
            <span class="main-featured-cards-btn-title">ellery x m`o capsule</span>
            <span class="main-featured-cards-btn-text">Known for her sculptural takes on
              traditional tailoring. Australian arbiter of cool Kym Ellery teams up with Moda
              Operandi.</span>
            <span class="main-featured-cards-btn-price">$52.00</span>
          </button>
          <button class="main-featured-cards-btn">
            <img class="main-featured-cards-btn-img" src="/img/clothes4.png" width="360" height="420">
            <span class="main-featured-cards-btn-title">ellery x m`o capsule</span>
            <span class="main-featured-cards-btn-text">Known for her sculptural takes on
              traditional tailoring. Australian arbiter of cool Kym Ellery teams up with Moda
              Operandi.</span>
            <span class="main-featured-cards-btn-price">$52.00</span>
          </button>
          <button class="main-featured-cards-btn">
            <img class="main-featured-cards-btn-img" src="/img/clothes5.png" width="360" height="420">
            <span class="main-featured-cards-btn-title">ellery x m`o capsule</span>
            <span class="main-featured-cards-btn-text">Known for her sculptural takes on
              traditional tailoring. Australian arbiter of cool Kym Ellery teams up with Moda
              Operandi.</span>
            <span class="main-featured-cards-btn-price">$52.00</span>
          </button>
          <button class="main-featured-cards-btn">
            <img class="main-featured-cards-btn-img" src="/img/clothes6.png" width="360" height="420">
            <span class="main-featured-cards-btn-title">ellery x m`o capsule</span>
            <span class="main-featured-cards-btn-text">Known for her sculptural takes on
              traditional tailoring. Australian arbiter of cool Kym Ellery teams up with Moda
              Operandi.</span>
            <span class="main-featured-cards-btn-price">$52.00</span>
          </button>
        </figure>
        <button class="main-featured-cards-all">Browse All Product</button>
      </section>
      <footer class="footer">
        <div class="footer-adv">
          <div class="footer-adv-pls">
            <img src="/img/footer1.svg" width="45" height="32">
            <span class="footer-adv-pls-title">Free Delivery</span>
            <span class="footer-adv-pls-text">Worldwide delivery on all. Authorit tively morph
              next-generation
              innov tion with extensive models.</span>
          </div>
          <div class="footer-adv-pls">
            <img src="/img/footer2.svg" width="45" height="32">
            <span class="footer-adv-pls-title">Sales &#38; discounts</span>
            <span class="footer-adv-pls-text">Worldwide delivery on all. Authorit tively morph
              next-generation
              innov tion with extensive models.</span>
          </div>
          <div class="footer-adv-pls">
            <img src="/img/footer3.svg" width="45" height="32">
            <span class="footer-adv-pls-title">Quality assurance</span>
            <span class="footer-adv-pls-text">Worldwide delivery on all. Authorit tively morph
              next-generation
              innov tion with extensive models.</span>
          </div>
        </div>
        <div class="footer-soc">
          <div class="footer-soc-fem">
            <img class="footer-soc-fem" src="/img/female-soc.png">
            <span class="footer-soc-fem-text">“Vestibulum quis porttitor dui! Quisque viverra nunc mi, a
              pulvinar
              purus condimentum“</span>
          </div>

          <div class="footer-soc-sub">
            <span class="footer-soc-sub-title">subscribe</span>
            <span class="footer-soc-sub-text">for our newsletter and promotion</span>
            <input class="footer-soc-sub-input" type="email" placeholder="Enter Your Email">
            <button class="footer-soc-sub-btn">subscribe</button>
          </div>
        </div>
        <div class="footer-cprgt">
          <div>© 2021 Brand All Rights Reserved.</div>
          <div class="footer-cprgt-socials">
            <image src="/img/fcbk.png"></image>
            <image src="/img/inst.png"></image>
            <image src="/img/twit.png"></image>
            <image src="/img/pint.png"></image>
          </div>
        </div>
      </footer>
    </main>
  </body>
</template>

<script setup>
</script>

<style lang="scss">
.menu {
    width: 1600px;
    margin: 0 auto;
    background-color: black;
    display: flex;
    justify-content: space-between;

    &-left {
        display: flex;
        padding: 10px 115px;
        box-sizing: border-box;

        &-logo {
            margin-right: 41px;
            width: 40px;
            height: 40px;
        }

        &-search {
            width: 26px;
        }
    }

    &-right {
        display: flex;
        align-items: center;
        padding: 10px 115px;

        &-burger {
            margin-right: 33px;
        }

        &-account {
            margin-right: 33px;
        }
    }





}

.main {
    width: 1600px;
    margin: 0 auto;

    &-featured {
        padding-bottom: 95px;
        display: flex;
        flex-wrap: wrap;
        flex-direction: column;
        align-content: center;
        justify-content: center;
        align-items: center;

        &-title {

            margin: 96px 0 0 0;
            color: #222222;
            text-transform: capitalize;
            font-size: 30px;
            text-align: center;
        }

        &-subtitle {
            color: #9F9F9F;
            font-size: 14px;
            text-align: center;
            margin: 6px;
        }

        &-cards {
            width: 1140px;
            margin: 0 auto;

            display: flex;
            flex-wrap: wrap;
            justify-content: space-between;
            align-items: center;
            align-self: center;

            &-btn {
                margin-top: 30px;
                cursor: pointer;
                border: none;


                &-title {
                    margin: 25px 18px 13px 18px;
                    display: block;
                    text-transform: uppercase;
                    text-align: start;
                    font-size: 13px;
                    font-weight: 400;
                    line-height: 16px;
                }

                &-text {
                    margin: 0 0 18px 16px;
                    text-align: start;
                    width: 314px;
                    display: block;
                    font-size: 14px;
                    line-height: 17px;
                    font-weight: 300;
                    color: #5d5d5d;
                }

                &-price {
                    margin: 0 0 20px 18px;
                    text-align: start;
                    display: block;
                    font-size: 16px;
                    color: #F16D7F;
                    line-height: 19px;
                    font-weight: 400;
                }


            }


            &-all {
                padding: 0px;
                display: block;
                justify-self: center;
                cursor: pointer;
                margin-top: 48px;
                width: 212px;
                height: 48px;
                background-color: transparent;
                color: #FF6A6A;
                border: solid 1px;
                font-size: 16px;
                line-height: 19px;
                font-weight: 400;
                border-color: #FF6A6A;
            }

            &-all:hover {
                background-color: #F16D7F;
                color: #ffffff;
            }
        }

        &-cards:hover {
            background-color: #3A3838DB;
        }
    }


    &-first {
        position: relative;
        display: flex;
        justify-content: end;
        background: url("/img/bg-main-block.png"), #f1e4e6;
        background-repeat: no-repeat;

        &-title {
            font-weight: 700;
            font-size: 32px;
            line-height: 38px;
            padding: 290px 330px 380px 0;
            box-sizing: border-box;
            text-transform: uppercase;

            &-words {
                display: block;
                font-weight: 900;
                font-size: 48px;
                line-height: 58px;
            }

            &-end {
                color: #f16d7f;
            }

            &-divider {
                position: absolute;
                width: 12px;
                height: 95px;
                background: #f16d7f;
                right: 755px;
            }
        }


    }

    &-discount {
        padding: 65px 230px;
        box-sizing: border-box;

        &-up {
            display: flex;
            justify-content: space-between;
            margin-bottom: 30px;

            &-button {
                width: 360px;
                padding: 105px 103px;
                box-sizing: border-box;
                border: none;
                background: rgba(0, 0, 0, 0.65) url("/img/wooman-card.png");
                background-blend-mode: darken;
                cursor: pointer;

                &-percent {
                    display: block;
                    font-weight: 400;
                    font-size: 16px;
                    line-height: 20px;
                    text-transform: uppercase;
                    color: #fff;
                }

                &-human {
                    display: block;
                    font-weight: 700;
                    font-size: 24px;
                    line-height: 29px;
                    text-transform: uppercase;
                    color: #f16d7f;
                }

                &:hover {
                    background-blend-mode: lighten;
                }
            }

            &>.main-discount-up-button:nth-child(2) {
                background: rgba(0, 0, 0, 0.65) url("/img/men-card.png");
            }

            &>.main-discount-up-button:last-child {
                background: rgba(0, 0, 0, 0.65) url("/img/child-card.png");
            }
        }

        &-down {
            width: 100%;
            padding: 65px;
            text-align: center;
            border: none;
            background: rgba(0, 0, 0, 0.65) url("/img/things-card.png");
            background-blend-mode: darken;
            cursor: pointer;

            &-percent {
                display: block;
                font-weight: 400;
                font-size: 16px;
                line-height: 20px;
                text-transform: uppercase;
                color: #fff;
            }

            &-text {
                display: block;
                font-weight: 700;
                font-size: 24px;
                line-height: 29px;
                text-transform: uppercase;
                color: #f16d7f;
            }

            &:hover {
                background-blend-mode: lighten;
            }
        }
    }
}

.footer {
    width: 1600px;

    &-adv {
        padding: 103px 230px;
        display: flex;
        background-color: #222224;

        &-pls {
            text-align: center;
            width: 360px;
            display: flex;
            margin-right: 30px;
            flex-direction: column;
            flex-wrap: wrap;
            justify-content: center;
            align-items: center;
            color: #FBFBFB;

            &-pic {
                width: auto;
                height: auto;
            }

            &-title {
                margin: 28px 0 16px 0;
                font-size: 20px;
                line-height: 24px;
                font-weight: 400;
            }

            &-text {
                font-size: 14px;
                line-height: 17px;
                font-weight: 400;
            }
        }
    }

    &-soc {
        display: flex;
        justify-content: center;
        align-items: center;
        background-image: url('/img/footer-soc.jpg');
        width: 1600px;
        padding-top: 97px;
        padding-bottom: 123px;

        &-fem {
            margin-bottom: 30px;
            display: flex;
            flex-wrap: wrap;
            align-items: center;
            justify-content: center;
            flex-direction: column;

            &-text {
                width: 360px;
                display: flex;
                font-size: 20px;
                font-weight: 400;
                line-height: 24px;
                text-align: center;
            }
        }

        &-sub {
            width: 557px;
            margin-left: 223px;
            padding: 0px;
            display: flex;
            flex-wrap: wrap;
            align-items: center;
            justify-content: center;

            &-title {
                display: flex;
                font-size: 24px;
                justify-self: center;
                text-transform: uppercase;
                font-weight: bold;
                line-height: 30px;
            }

            &-text {
                display: flex;
                font-size: 24px;
                justify-self: center;
                text-transform: uppercase;
                font-weight: 400;
                line-height: 30px;
                margin-bottom: 32px;
            }

            &-input {
                padding-left: 22px;
                border-radius: 20px 0px 0px 20px;
                border: 0px;
                width: 256px;
                height: 50px;
            }

            &-btn {
                cursor: pointer;
                text-transform: capitalize;
                border-radius: 0px 20px 20px 0px;
                border: 0px;
                width: 100px;
                height: 50px;
                background-color: #F16D7F;
            }
        }
    }

    &-cprgt {
        display: flex;
        width: 1600px;
        background-color: #222224;
        color: #FBFBFB;
        justify-content: space-around;
        align-items: center;

        &-socials {
            padding: 24px;
        }
    }
}




@media(max-width: 768px) {
    .menu {
        width: 768px;
        display: flex;
        justify-content: space-between;
        align-items: center;




        &-right {
            padding: 5px 55px;
            display: flex;
            justify-content: flex-end;
            align-items: center;
        }

        &-left {
            padding: 5px 55px;
            display: flex;
            justify-content: flex-start;
        }

    }

    .main {
        width: 768px;

        &-first {
            background-size: 368px 368px;

            &-title {
                padding: 135px 33px 135px 0px;

                &-words {
                    line-height: 53px;
                    font-size: 44px;
                }

                &-start {
                    font-size: 24px;
                    line-height: 29px;
                }

                &-end {
                    font-size: 24px;
                    line-height: 29px;
                }

                &-divider {
                    right: 350px;
                }

            }
        }

        &-discount {
            padding: 16px;
            display: block;

            &-up {
                display: flex;
                flex-wrap: wrap;
                justify-content: space-between;


                &-button {
                    width: 232px;
                    padding: 45px 43px;
                    background-size: contain;
                }

                &>.main-discount-up-button:nth-child(2) {
                    background-size: contain;
                }

                &>.main-discount-up-button:nth-child(3) {
                    background-size: contain;
                }


            }

            &-down {
                padding: 33px;
            }
        }

        &-featured {
            &-cards {
                justify-content: space-around;
                width: 768px;

            }
        }
    }

    .footer {
        width: 768px;

        &-adv {
            flex-direction: column;

            &-pls {
                margin-bottom: 48px;
            }
        }

        &-soc {
            width: 768px;
            background-position: 25% 85%;
            background-size: cover;
            background-repeat: no-repeat;
            display: flex;
            padding: 0px;
            flex-wrap: wrap;
            flex-direction: column;

            &-fem {
                margin-top: 64px;
                margin-bottom: 48px;
                display: flex;
                flex-wrap: wrap;
                align-items: center;
                justify-content: center;
                flex-direction: column;


                &-text {
                    margin-right: 0px;
                }
            }

            &-sub {
                margin-bottom: 140px;
            }

            &-sub {
                margin-left: 0px;
            }
        }

        &-cprgt {
            width: 768px;
        }

    }
}




@media(max-width: 375px) {
    .menu {
        width: 375px;
        padding: 0px;
        display: flex;
        padding-top: 18px;
        padding-bottom: 18px;

        &-right {
            padding: 0px;
            display: flex;
            justify-content: flex-end;
            align-items: center;

            &-account {
                display: none;
            }

            &-cart {
                display: none;
            }
        }

        &-left {
            padding: 0px;
            margin-left: 16px;
            display: flex;
            justify-content: flex-start;
        }
    }

    .main {
        width: 375px;

        &-first {
            width: 375px;
            display: flex;
            background-image: none;

            &-title {
                padding: 149px 45px 149px 85px;
                display: flex;
                flex-wrap: wrap;
                justify-content: space-between;
                align-items: center;

                &-start {
                    line-height: 24px;
                    font-size: 20px;
                }

                &-end {
                    line-height: 24px;
                    font-size: 20px;
                }

                &-words {
                    line-height: 47px;
                    display: flex;
                    font-size: 38px;
                }

                &-divider {
                    right: 300px;
                    height: 61px;
                }
            }
        }

        &-discount {
            padding: 16px;

            &-up {
                display: flex;
                justify-content: center;
                margin-bottom: 0px;

                &-button {
                    width: 343px;
                    padding: 100px 43px;
                    margin-bottom: 32px;
                }
            }
        }

        &-featured {
            width: 375px;

            &-cards {
                width: 375px;

            }

        }
    }

    .footer {
        width: 375px;

        &-adv {
            width: 375px;
            padding: 48px 0px 0px 0px;
        }

        &-soc {
            padding-top: 0px;
            padding-bottom: 0px;
            width: 375px;

            &-sub {
                width: 341px;
                display: flex;

                &-input {
                    width: auto;
                }

                &-text {
                    font-size: 14px;
                }
            }
        }

        &-cprgt {
            display: flex;
            flex-direction: column-reverse;
            width: 375px;

            &-socials {
                margin-top: 43px;
                margin-bottom: 40px;
            }
        }
    }
}
</style>
